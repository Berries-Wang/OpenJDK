...
/**
 *
 * 1.JVM在运行的时候，需要一种用来表示Java内部类型的机制，在HotSpot中的解决方案是:
 *为每一个Java类创建一个instanceKlass对象，
...
//    [EMBEDDED host klass        ] only exist for an anonymous class (JSR 292
//    enabled)
...
#ifndef PRODUCT
// Print fields.
// If "obj" argument to constructor is NULL, prints static fields, otherwise
// prints non-static fields.
...
protected:
  // Constructor
  InstanceKlass(int vtable_len, int itable_len, int static_field_size,
                int nonstatic_oop_map_size, ReferenceType rt,
                AccessFlags access_flags, bool is_anonymous);

public:
  static InstanceKlass *
  allocate_instance_klass(ClassLoaderData *loader_data, int vtable_len,
                          int itable_len, int static_field_size,
                          int nonstatic_oop_map_size, ReferenceType rt,
                          AccessFlags access_flags, Symbol *name,
                          Klass *super_klass, bool is_anonymous, TRAPS);

  InstanceKlass() {
    assert(DumpSharedSpaces || UseSharedSpaces, "only for CDS");
  }

  // See "The Java Virtual Machine Specification" section 2.16.2-5 for a
  // detailed description of the class loading & initialization
  // procedure（程序、手續、步骤）, and the use of the states.
  enum ClassState {
...
protected:
  ...
  int _static_field_size; // number words used by static fields (oop and
                          // non-oop) in this klass
  ...
  enum {
    ...
    _misc_declares_default_methods =
        1 << 6 // directly declares default methods (any access)
  };
  u2 _misc_flags;
  u2 _minor_version;    // minor version number of class file
  u2 _major_version;    // major version number of class file
  Thread *_init_thread; // Pointer to current thread doing initialization (to
                        // handle recusive initialization)
  int _vtable_len;      // length of Java vtable (in words)
  int _itable_len;      // length of Java itable (in words)
  OopMapCache *volatile _oop_map_cache; // OopMapCache for all methods in the
                                        // klass (allocated lazily)
  MemberNameTable *_member_names;       // Member names
  JNIid *_jni_ids; // First JNI identifier for static fields in this class
  jmethodID *_methods_jmethod_ids; // jmethodIDs corresponding to method_idnum,
                                   // or NULL if none
  nmethodBucket *_dependencies;    // list of dependent nmethods
  nmethod *_osr_nmethods_head;  // Head of list of on-stack replacement nmethods
                                // for this class
  ...
  volatile u2 _idnum_allocated_count; // JNI/JVMTI: increments with the addition
                                      // of methods, old ids don't change
  ...
  JvmtiCachedClassFieldMap
      *_jvmti_cached_class_field_map; // JVMTI: used during heap iteration
...
public:
  ...
  void set_default_vtable_indices(Array<int> *v) {
    _default_vtable_indices = v;
  }
  ...
  Array<Klass *> *transitive_interfaces() const {
...
private:
  ...
  FieldInfo *field(int index) const {
    return FieldInfo::from_field_array(_fields, index);
  }

public:
  int field_offset(int index) const { return field(index)->offset(); }
  int field_access_flags(int index) const {
    return field(index)->access_flags();
  }
  Symbol *field_name(int index) const {
    return field(index)->name(constants());
  }
  Symbol *field_signature(int index) const {
  ...
  enum InnerClassAttributeOffset {
    // From
    // http://mirror.eng/products/jdk/1.1/docs/guide/innerclasses/spec/innerclasses.doc10.html#18814
  ...
  bool is_override(methodHandle super_method, Handle targetclassloader,
                   Symbol *targetclassname, TRAPS);

  // package
  bool is_same_class_package(Klass *class2);
  bool is_same_class_package(oop classloader2, Symbol *classname2);
  static bool is_same_class_package(oop class_loader1, Symbol *class_name1,
  ...
  // find field in direct superinterfaces, returns the interface in which the
  // field is defined
  Klass *find_interface_field(Symbol *name, Symbol *sig,
                              fieldDescriptor *fd) const;
  // find field according to JVM spec 5.4.3.2, returns the klass in which the
  // field is defined
  Klass *find_field(Symbol *name, Symbol *sig, fieldDescriptor *fd) const;
  // find instance or static fields according to JVM spec 5.4.3.2, returns the
  // klass in which the field is defined
  ...
  bool find_local_field_from_offset(int offset, bool is_static,
                                    fieldDescriptor *fd) const;
  ...
  static Method *find_method(Array<Method *> *methods, Symbol *name,
                             Symbol *signature);

  // find a local method, but skip static methods
  Method *find_instance_method(Symbol *name, Symbol *signature);
  static Method *find_instance_method(Array<Method *> *methods, Symbol *name,
                                      Symbol *signature);

  // true if method matches signature and conforms to skipping_X conditions.
  static bool method_matches(Method *m, Symbol *signature,
                             bool skipping_overpass, bool skipping_static);

  // find a local method index in default_methods (returns -1 if not found)
  static int find_method_index(Array<Method *> *methods, Symbol *name,
                               Symbol *signature, bool skipping_overpass,
                               bool skipping_static);

  // lookup operation (returns NULL if not found)
  Method *uncached_lookup_method(Symbol *name, Symbol *signature,
  ...
  // (returns NULL if not found)
  Method *lookup_method_in_ordered_interfaces(Symbol *name,
  ...
  static int find_method_by_name(Array<Method *> *methods, Symbol *name,
                                 int *end);
  ...
  Klass *host_klass() const {
    ...
    if (hk == NULL) {
      ...
      assert(*hk != NULL,
             "host klass should always be set if the address is not null");
      return *hk;
    }
  }
  ...
  bool is_anonymous() const { return (_misc_flags & _misc_is_anonymous) != 0; }
  ...
  Symbol *source_file_name() const {
    return (_source_file_name_index == 0)
               ? (Symbol *)NULL
               : _constants->symbol_at(_source_file_name_index);
  }
  ...
  // JVMTI: Support for caching a class file before it is modified by an agent
  // that can do retransformation
  ...
  Symbol *generic_signature() const {
    return (_generic_signature_index == 0)
               ? (Symbol *)NULL
               : _constants->symbol_at(_generic_signature_index);
  }
  ...
  static jmethodID
  get_jmethod_id_fetch_or_update(instanceKlassHandle ik_h, size_t idnum,
  ...
  AnnotationArray *class_type_annotations() const {
    return (_annotations != NULL) ? _annotations->class_type_annotations()
                                  : NULL;
  }
  Array<AnnotationArray *> *fields_type_annotations() const {
    return (_annotations != NULL) ? _annotations->fields_type_annotations()
                                  : NULL;
  }
  // allocation
  ...
  nmethod *lookup_osr_nmethod(const Method *m, int bci, int level,
  ...
  void
  add_implementor(Klass *k); // k is a new class that implements this interface
  ...
  void do_local_static_fields(void f(fieldDescriptor *, Handle, TRAPS), Handle,
                              TRAPS);
  ...
  static int size(int vtable_length, int itable_length,
                  int nonstatic_oop_map_size, bool is_interface,
                  bool is_anonymous) {
    return align_object_size(
        header_size() + align_object_offset(vtable_length) +
...
#if INCLUDE_ALL_GCS
#define InstanceKlass_OOP_OOP_ITERATE_BACKWARDS_DECL(OopClosureType,           \
                                                     nv_suffix)                \
...
private:
  ...
  // Static methods that are used to implement member methods where an exposed
  // this pointer is needed due to possible GCs
  static bool link_class_impl(instanceKlassHandle this_oop,
                              bool throw_verifyerror, TRAPS);
  static bool verify_code(instanceKlassHandle this_oop, bool throw_verifyerror,
                          TRAPS);
  ...
  static void
  set_initialization_state_and_notify_impl(instanceKlassHandle this_oop,
                                           ClassState state, TRAPS);
  static void call_class_initializer_impl(instanceKlassHandle this_oop, TRAPS);
  static Klass *array_klass_impl(instanceKlassHandle this_oop, bool or_null,
                                 int n, TRAPS);
  static void do_local_static_fields_impl(instanceKlassHandle this_oop,
  ...
  Method *find_method_impl(Symbol *name, Symbol *signature,
                           bool skipping_overpass) const;
  static Method *find_method_impl(Array<Method *> *methods, Symbol *name,
                                  Symbol *signature, bool skipping_overpass,
                                  bool skipping_static);
...
public:
  ...
  virtual void restore_unshareable_info(ClassLoaderData *loader_data,
  ...
  void set_member_names(MemberNameTable *member_names) {
...
public:
  ...
  bool find_local_field(fieldDescriptor *fd) {
    return InstanceKlass::cast(holder())->find_local_field_from_offset(
        offset(), true, fd);
  }
...
public:
  ...
  ~PreviousVersionNode();
  ConstantPool *prev_constant_pool() const { return _prev_constant_pool; }
...
public:
  ...
  void next() { _idx += InstanceKlass::inner_class_next_offset; }

  bool done() const { return (_idx >= _length); }
  ...
  u2 inner_name_index() const {
    return _inner_classes->at(_idx +
                              InstanceKlass::inner_class_inner_name_offset);
  }

  void set_inner_name_index(u2 index) {
    _inner_classes->at_put(_idx + InstanceKlass::inner_class_inner_name_offset,
                           index);
  }
