            ...
            if (callerCL == null) {
              /*
                ...
                给自己看的:
                   ...
                   需要的时候取出去加载类或者是取类出来使用。而是直接通过线程上下文类加载器(应用程序类加载器/其他手动设置
                   进去的类加载器。因为类似于mysql驱动这些(java.sql是java核心类库，存在于rt.jar下，由启动类加载器加载。但是
                   又需要调用用户程序目录下的代码[实现的代码，如获取mysql 的connection],而这部分代码只能由程序类加载器去加载)，
                   存在于用户程序目录下，启动类加载器&拓展类加载器无法加载到，
                   只能通过应用程序类加载器/其他用户自定义类加载器去加载)去加载/取出类
