...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
     ...
     */
    static final int MOVED = -1; // hash for forwarding nodes,表明正在扩容
    static final int TREEBIN = -2; // hash for roots of trees
    static final int RESERVED = -3; // hash for transient reservations

    /***
     * 
     * 0x7fffffff对应的二进制: 01111111111111111111111111111111
     */
     ...
     * 下舍入到下一个2的幂，见《高效程序的奥秘》3.2小节：上舍入、下舍入到下一个2的幂 See Hackers Delight, sec 3.2
     */
    ...
    /***
     * Table initialization and resizing control. When negative(adj. [数]
     * 负的；消极的；否定的；阴性的), the table is being initialized or resized: -1 for
     * initialization, else -(1 + the number of active resizing threads). Otherwise,
     * when table is null, holds the initial table size to use upon creation, or 0
     * for default. After initialization, holds the next element count value upon
     ...
     * 2. 其他情况 
     *     a. 当table为空,0代表默认值，否则是代表table的容量 
     *     b. 当table初始化之后，代表的是扩容的阈值
     * </pre>
     */
     ...
     * 
     * 转移开始下标，从后往前转移，即tab[transferIndex,length-1]均已经转移成功
     */
    private transient volatile int transferIndex;

    /**
     * Spinlock(自旋锁) (locked via CAS) used when resizing and/or creating
     * CounterCells.
     */
     ...
     * 
     * 为什么不像HashMap直接检查桶的根节点呢?也是一项优化项吧
     */
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        /***
         * 这里的获取hash的方式与java.util.HashMap有所不同 1. 这里直接不允许Key为null，但是HashMap可以 2.
         * 在spread方法中会多一个& HASH_BITS操作
         * 
         */
        ...
        // 其实这也是一个自旋操作，元素必须插入到Map中
        for (Node<K, V>[] tab = table;;) {

            Node<K, V> f;
            int n, i, fh;
            // 当table为null或者初始容量为0，则需要去初始化table
            if (tab == null || (n = tab.length) == 0) {
                // 初始化table
                tab = initTable();
            } else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) { // 根据hash值从table中获取对应位置上的元素，如果为空，则表明该位置没有元素
                // 元素为空，表明该位置还没有元素，直接赋值即可，当设置成功，可以直接返回了；若设置失败，则进行自旋
                if (casTabAt(tab, i, null, new Node<K, V>(hash, key, value, null))) {
                    break; // no lock when adding to empty bin
                }
            } else if ((fh = f.hash) == MOVED) { // 即对应的桶的数据被迁移了，整个CHM都在扩容，此时数据无法插入，当前的插入线程也会协助进行扩容,因为是自旋，所以协助扩容完成之后才会继续执行插入操作
                tab = helpTransfer(tab, f);
            } else {
                V oldVal = null;
                // 使用synchronized加锁，注意锁的是什么,细化锁的粒度。 这里锁的是单个桶
                synchronized (f) {
                    // 这里再判断一下，因为扩容了，i位置上的元素可能就不是f了
                    if (tabAt(tab, i) == f) {
                        // 如果大于0，则说明是一个链表结构
                        if (fh >= 0) {
                            binCount = 1;
                            for (Node<K, V> e = f;; ++binCount) {
                                ...
                                Node<K, V> pred = e;
                                // Key不存在，则将元素插入到链表末尾。（尾插）
                                if ((e = e.next) == null) {
                                    pred.next = new Node<K, V>(hash, key, value, null);
                                    break;
                                }
                            }
                        } else if (f instanceof TreeBin) { // 当桶的类型是一个树结构
                            Node<K, V> p;
                            binCount = 2;
                            // 将值添加到红黑树中
                ...
                if (binCount != 0) {
                    // 进行树化操作，但是条件不仅仅如此，需要查看treeifyBin方法
    ...
    /**
     * A node inserted at head of bins during transfer
     * operations.(该类的Node被插入到容器的头部在转移操作期间)
     */
    ...
    private final Node<K, V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            // sizeCtl<0表示正在初始化或者扩容，即可能其他线程在初始化或者扩容，因此需要Thread.yield()
            if ((sc = sizeCtl) < 0) {
                /**
                 * Thread.yield() 方法，使当前线程由执行状态，变成为就绪状态，让出cpu时间，在下一个线程执行时候，此线程有可能被执行，也有可能没有被执行。
                 * 
                 * 如果这里不重新声明一个变量，那么U.compareAndSwapInt(this, SIZECTL, sc, -1)返回的永远为True了。
                 * 也就是说一定会进入else if (U.compareAndSwapInt(this, SIZECTL, sc, -1))分支
                 */
                Thread.yield(); // lost initialization race; just spin
            } else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
                /**
                 * U.compareAndSwapInt(this, SIZECTL, sc, -1) 使用CAS "无锁编程"
                 * 的方式来获取扩容的锁(>>>>注意，CAS不是锁<<<<) 这里CAS操作失败，那么就会返回false，此时会开启下一次循环。
                 * 
                 * 这里使用 CAS + 自旋 的方式来进行初始化table的
                 */
                try {
                    /**
                     * 双重校验的方式，更加安全.
                     * 
                     * 因为其他的进入到了该分支的线程一直在自旋，自旋的线程还是有机会执行到这里的，因此需要做一个判断。 如： A线程执行到sizeCtl =
                     * sc;之前，B线程自旋经过while的条件判断,这是一个B执行到这里的一个机会。
                     */
     ...
     * @param check if <0, don't check resize, if <= 1 only check if
     *              uncontended.(当添加元素和移除元素的时候check才会大于0)
     * 
     *              可以看到，这里使用的是CAS操作，即无锁编程
     */
    private final void addCount(long x, int check) {
         ...
         * 首先在baseCount的基础上添加， 添加成功则进入下一步；反之，则需要在CounterCell里添加计数
         */
        if ((as = counterCells) != null || !U.compareAndSwapLong(this, BASECOUNT, b = baseCount, s = b + x)) {
             ...
             * 从CounterCell中获取对应的counterCell,当前线程所增加的计数也是在这个CounterCell里添加(结合 获取元素数量的方法分析)
             * 
             * 获取当前线程对应的CounterCell并尝试使用CAS来增加计数，成功则返回，反之，则进行fullAddCount
             * 
             */
            if (as == null || (m = as.length - 1) < 0 || (a = as[ThreadLocalRandom.getProbe() & m]) == null
                ...
                /**
                 * 1. CouterCell为null； 2.当前线程对应的CounterCell为空； 3.CAS操作失败 三种情况下会进入fullAddCount方法
                 */
        ...
        // 当添加或者移除元素时，check会大于0
        if (check >= 0) {
            Node<K, V>[] tab, nt;
            int n, sc;

            /**
             * sizeCtl当为正整数且table初始化了，则表示扩容的阈值
             * 
             * 即： 若CHM的元素数量达到了扩容的阈值且table不为空且容量小于最大容量
             * 
             * 另外一层意思就是当s=sumCount执行完成，s >= (long) (sc = sizeCtl执行之前，在这期间，扩容完成，则也是不需要扩容的。
             * 
             * 注意，这里是自旋，即使当transfer(tab, nt);返回，但是while条件依旧满足，还是需要协助扩容的
             */
            while (s >= (long) (sc = sizeCtl) && (tab = table) != null && (n = tab.length) < MAXIMUM_CAPACITY) {

                // 需要看一下方法注释, 其实这里是计算一个基数
                int rs = resizeStamp(n);
                // 为什么需要判断小于0?,表示要么在扩容，要么在初始化
                if (sc < 0) {
                    /**
                     * <pre>
                     * 1. sc >>> RESIZE_STAMP_SHIFT) != rs，即应该是没有线程在扩容了
                     * 2. sc == rs + 1  
                     * 3. sc == rs + MAX_RESIZERS 是否达到最大扩容的线程数
                     * 4. (nt = nextTable) == null 判断是否迁移完成
                     * 5. transferIndex <= 0  判断是否迁移完成
                     * </pre>
                     * 
                     * 即当扩容完成或者扩容线程已达上限
                     */
                    ...
                    if (U.compareAndSwapInt(this, SIZECTL, sc, sc + 1)) { // 将sizeCtl设置为sizeCtl+1，即表示新增一个线程进行扩容
                        // 协助扩容
                        transfer(tab, nt);
                    }

                } else if (U.compareAndSwapInt(this, SIZECTL, sc, (rs << RESIZE_STAMP_SHIFT) + 2)) { // 第一次进入，是需要扩容的，因此会走这里
                    // 进行主动扩容操作(第二个参数来判断主动扩容还是协助扩容)
                    transfer(tab, null);
                }
    ...
    private final void transfer(Node<K, V>[] tab, Node<K, V>[] nextTab) {
        ...
        // 当计算出来的处理桶的数量小于MIN_TRANSFER_STRIDE，则本次处理桶的数量取MIN_TRANSFER_STRIDE
        ...
        if (nextTab == null) { // initiating
            try {
                @SuppressWarnings("unchecked")
                // 创建一个新的桶数组
                Node<K, V>[] nt = (Node<K, V>[]) new Node<?, ?>[n << 1]; // 移位操作代替乘法
                nextTab = nt;
            ...
            // nextTable为成员变量
            nextTable = nextTab;
            // 注意成员变量transferIndex,转移开始的下标
            transferIndex = n;
        }
        int nextn = nextTab.length;
        // 创建一个ForwardingNode类型的节点
        ...
        for (int i = 0, bound = 0;;) {
            ...
            while (advance) {
                ...
                // --i >= bound表示扩容完全完成，即数据全部转移完成
                if (--i >= bound || finishing) {
                    advance = false;
                } else if ((nextIndex = transferIndex) <= 0) { // 小于0表示数据全部转移完成
                    i = -1;
                    advance = false;
                } else if (U.compareAndSwapInt(this, TRANSFERINDEX, nextIndex,
                        nextBound = (nextIndex > stride ? nextIndex - stride : 0))) { // 更新transferIndex属性的值,获取当前线程需要转移的范围
                    // nextBound是一个关键值，即下次扩容时的开始下标,即本线程负责[bound,i]之间的数据转移
                    bound = nextBound;
            ...
            if (i < 0 || i >= n || i + n >= nextn) {
                ...
                if (finishing) {
                    nextTable = null;
                    table = nextTab;
                    // sizeCtl = 2n -(1/2)n = 2n * (3/4)n 即通过移位操作来计算应用负载率，即重新计算扩容阈值
                    sizeCtl = (n << 1) - (n >>> 1);
                    return;
                }

                // 当前线程负责的数据迁移完成，迁移线程数减一
                if (U.compareAndSwapInt(this, SIZECTL, sc = sizeCtl, sc - 1)) {
                    // 判断是否还有线程在迁移数据，有，则返回，让上层来判断是否需要继续协助扩容
            ...
            } else if ((f = tabAt(tab, i)) == null) { // 当桶里没有元素，则不需要迁移，设置节点为ForwardingNode类型的节点，代表该桶迁移完成
                advance = casTabAt(tab, i, null, fwd);
            } else if ((fh = f.hash) == MOVED) { // 对应桶中的元素迁移完成，单nexttable 还没有赋值给table,即代表CHM正在扩容
                advance = true; // already processed
            } else {
                synchronized (f) { // 锁住桶中的元素，进行数据迁移
                    if (tabAt(tab, i) == f) {
                        Node<K, V> ln, hn;
                        if (fh >= 0) {
                            int runBit = fh & n;
    ...
    private final void fullAddCount(long x, boolean wasUncontended) {
        ...
        for (;;) {
            ...
            if ((as = counterCells) != null && (n = as.length) > 0) {
                ...
                if ((a = as[(n - 1) & h]) == null) {// 当CounterCell不存在时
                    if (cellsBusy == 0) { // Try to attach new Cell
                        // 创建一个新的CounterCell并添加到CounterCell数组中
                    ...
                    // 以下是当前线程对应的CounterCell已经存在了
                } else if (!wasUncontended) {// CAS already known to fail
                    // 重新哈希,wasUncontended
                    wasUncontended = true; // Continue after rehash
                } else if (U.compareAndSwapLong(a, CELLVALUE, v = a.value, v + x)) { // 使用CAS操作CounterCell增加计数,成功则返回
                    break;
                } else if (counterCells != as || n >= NCPU) { // 当CounterCell数组长度大于等于CPU的数量的时候，就没有必要扩容了
                    collide = false; // At max size or stale
                } else if (!collide) {// 能够执行到这里，其实就已经有竞争了，所以需要重新哈希
                    collide = true;
            ...
            } else if (cellsBusy == 0 && counterCells == as && U.compareAndSwapInt(this, CELLSBUSY, 0, 1)) { // 初始化counterCells
                boolean init = false;
            ...
            } else if (U.compareAndSwapLong(this, BASECOUNT, v = baseCount, v + x)) { // 使用CounterCell失败，退一步，尝试使用baseCount
