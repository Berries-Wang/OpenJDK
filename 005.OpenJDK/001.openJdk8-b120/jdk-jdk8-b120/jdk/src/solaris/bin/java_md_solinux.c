...
void CreateExecutionEnvironment(int *pargc, char ***pargv, char jrepath[],
                                jint so_jrepath, char jvmpath[],
                                jint so_jvmpath, char jvmcfg[],
                                jint so_jvmcfg) {
  /*
  ...
  /* Check data model flags, and exec process, if needed */
  {
    ...
    { /* open new scope to declare local variables */
      ...
      for (i = 1; i < argc; i++) {
        ...
        if (JLI_StrCmp(argv[i], "-J-d64") == 0 ||
            JLI_StrCmp(argv[i], "-d64") == 0) {
          wanted = 64;
          continue;
        }
        if (JLI_StrCmp(argv[i], "-J-d32") == 0 ||
            JLI_StrCmp(argv[i], "-d32") == 0) {
          wanted = 32;
          continue;
        }
        ...
        if (IsJavaArgs()) {
          ...
          if (JLI_StrCmp(argv[i], "-classpath") == 0 ||
              JLI_StrCmp(argv[i], "-cp") == 0) {
            i++;
            if (i >= argc)
              break;
            newargv[newargc++] = argv[i];
    ...
    if (wanted == running) {
      ...
      JLI_Snprintf(jvmcfg, so_jvmcfg, "%s%slib%s%s%sjvm.cfg", jrepath, FILESEP,
                   FILESEP, arch, FILESEP);
      ...
      if (running != wanted) {
        ...
        JLI_Snprintf(jvmcfg, so_jvmcfg, "%s%slib%s%s%sjvm.cfg", jrepath,
                     FILESEP, FILESEP, GetArchPath(wanted), FILESEP);
        /*
    ...
    if (mustsetenv) {
      ...
      new_runpath =
          JLI_MemAlloc(((runpath != NULL) ? JLI_StrLen(runpath) : 0) +
       ...
       */
      {
        ...
        sprintf(new_runpath,
                ...
                jvmpath,
#ifdef DUAL_MODE
                jrepath, GetArchPath(wanted), jrepath, GetArchPath(wanted)
#else  /* !DUAL_MODE */
       ...
       */
      if (dmpath != NULL)
        (void)UnsetEnv((wanted == 32) ? "LD_LIBRARY_PATH_32"
                                      : "LD_LIBRARY_PATH_64");
#endif /* __solaris */
...
#endif /* SETENV_REQUIRED */
    {
      ...
      if (running != wanted) {
        char *oldexec =
            JLI_StrCpy(JLI_MemAlloc(JLI_StrLen(execname) + 1), execname);
        char *olddir = oldexec;
...
jboolean LoadJavaVM(const char *jvmpath, InvocationFunctions *ifn) {
  ...
  /**
   * dlopen 用于加载动态链接库libjvm.so，使用man page可以查看
   * 
   */ 
  ...
  if (libjvm == NULL) {
    ...
    // 从动态链接库中加载函数: JNI_CreateJavaVM,定义于: 005.OpenJDK/001.openJdk8-b120/jdk-jdk8-b120/hotspot/src/share/vm/prims/jni.cpp
...
int JVMInit(InvocationFunctions *ifn, jlong threadStackSize, int argc,
