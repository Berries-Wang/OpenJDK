...
//
// A Klass provides:
//  1: language level class object (method dictionary
...
/**
 * OOP 全称是Ordinary Object
 * Pointer，即普通对象指针。在JVM中，oop对应的类是oopDesc，每new一个Java实例就会创建一个oopDesc，oopDesc包含了两部分信息，_mark
 * 和 _metadata,markword是用来
 * 存储对象的锁信息和分代年龄用的，_metadata就是指向Klass的一个指针，通过指针可以拿到这个类的数据
 * 即《深入理解JVM》中的Mark
 * Work(存储对象自身的运行时的数据，包括hashCode，GC分代年龄，锁状态标识，线程持有锁，偏向线程id，偏向时间戳等)
 * 和类型指针(指向其类型元数据的指针)
 */
...
protected:
  ...
  // for better cache behavior (may not make much of a difference but sure won't
  // hurt)
  /*
  ...
  // The "layout helper" is a combined descriptor of object layout.layout
  // helper是对象布局的组合描述符 For klasses which are neither instance nor
  // array, the value is zero. 表示对象时，既不是对象实例也不是数组，他的值为零
  //

  /**
   * 对于对象而言,layout
   * helper是一个正数，这个对象实例的大小。这个大小已经通过align_object_size传递并缩放到字节
  ...
  // where:
  //    tag is 0x80 if the elements are oops, 0xC0 if
  //    non-oops(1个字节，若值为0x80，表示数组元素的类型是OOP；值为0xC0,则表示数组
  // 元素的类型为Java基本类型)
  //    hsz is array header size in bytes (i.e., offset of first
  //    element)(数组头部大小，以字节为单位，表示第一个元素的偏移) ebt is the
  //    BasicType of the elements，基本类型的元素 esz is the element size in
  //    bytes 元素大小，以字节为单位
  ...
  // and _primary_supers all help make fast subtype
  // checks(都是为了快速进行子类型的检查).  See big discussion in
  // doc/server_compiler/checktype.txt
  //
   ...
   */
  jint
      _modifier_flags; // Processed access flags, for use by Class.getModifiers.
  /**
   * 获取类的修饰符，如private类访问控制，final，static，abstract ，native等
   */
  AccessFlags _access_flags; // Access flags. The class/interface distinction is
                             // stored here.
...
public:
  ...
  // java_super is the Java-level super type as specified by
  // Class.getSuperClass.
  ...
  // Returns number of primary supers; may be a number in the inclusive range
  // [0, primary_super_limit].
  juint super_depth() const {
    if (!can_be_primary_super()) {
      return primary_super_limit();
    } else {
      juint d = (super_check_offset() - in_bytes(primary_supers_offset())) /
                sizeof(Klass *);
  ...
  void append_to_sibling_list(); // add newly created receiver to superklass'
                                 // subklass list

  void set_next_link(Klass *k) { _next_link = k; }
  Klass *next_link() const {
    return _next_link;
  } // The next klass defined by the class loader.
  ...
  void accumulate_modified_oops() {
    if (has_modified_oops())
      _accumulated_modified_oops = 1;
  }
  void clear_accumulated_modified_oops() { _accumulated_modified_oops = 0; }
  bool has_accumulated_modified_oops() {
    return _accumulated_modified_oops == 1;
  }
...
public:
  // Compiler support
  static ByteSize super_offset() {
    return in_ByteSize(offset_of(Klass, _super));
  }
  static ByteSize super_check_offset_offset() {
    return in_ByteSize(offset_of(Klass, _super_check_offset));
  }
  static ByteSize primary_supers_offset() {
    return in_ByteSize(offset_of(Klass, _primary_supers));
  }
  static ByteSize secondary_super_cache_offset() {
    return in_ByteSize(offset_of(Klass, _secondary_super_cache));
  }
  static ByteSize secondary_supers_offset() {
    return in_ByteSize(offset_of(Klass, _secondary_supers));
  }
  static ByteSize java_mirror_offset() {
    return in_ByteSize(offset_of(Klass, _java_mirror));
  }
  static ByteSize modifier_flags_offset() {
    return in_ByteSize(offset_of(Klass, _modifier_flags));
  }
  static ByteSize layout_helper_offset() {
  ...
  static bool layout_helper_is_typeArray(jint lh) {
    ...
    return (juint)lh >=
           (juint)(_lh_array_tag_type_value << _lh_array_tag_shift);
  }
  ...
  static jint array_layout_helper(jint tag, int hsize, BasicType etype,
                                  int log2_esize) {
    return (tag << _lh_array_tag_shift) | (hsize << _lh_header_size_shift) |
  ...
  // subtype check: true if is_subclass_of, or if k is interface and receiver
  // implements it
  ...
  // array copying
  virtual void copy_array(arrayOop s, int src_pos, arrayOop d, int dst_pos,
                          int length, TRAPS);
  ...
  virtual Klass *find_field(Symbol *name, Symbol *signature,
                            fieldDescriptor *fd) const;
  virtual Method *uncached_lookup_method(Symbol *name, Symbol *signature,
...
public:
  ...
  virtual bool is_leaf_class() const {
    fatal("not a class");
    return false;
  }
...
#undef assert_same_query
  ...
  bool has_vanilla_constructor() const {
    return _access_flags.has_vanilla_constructor();
  }
  void set_has_vanilla_constructor() {
    _access_flags.set_has_vanilla_constructor();
  }
  bool has_miranda_methods() const {
    return access_flags().has_miranda_methods();
  }
  ...
  static ByteSize prototype_header_offset() {
    return in_ByteSize(offset_of(Klass, _prototype_header));
  }

  int biased_lock_revocation_count() const {
    return (int)_biased_lock_revocation_count;
  }
  // Atomically increments biased_lock_revocation_count and returns updated
  // value
  int atomic_incr_biased_lock_revocation_count();
  void set_biased_lock_revocation_count(int val) {
    _biased_lock_revocation_count = (jint)val;
  }
  jlong last_biased_lock_bulk_revocation_time() {
    return _last_biased_lock_bulk_revocation_time;
  }
  void set_last_biased_lock_bulk_revocation_time(jlong cur_time) {
    _last_biased_lock_bulk_revocation_time = cur_time;
  }
  ...
  virtual int oop_oop_iterate_m(oop obj, ExtendedOopClosure *blk,
                                MemRegion mr) = 0;
  virtual int oop_oop_iterate_v_m(oop obj, ExtendedOopClosure *blk,
                                  MemRegion mr) {
...
#define Klass_OOP_OOP_ITERATE_DECL(OopClosureType, nv_suffix)                  \
  ...
  /* Iterates "blk" over all the oops in "obj" (of type "this") within "mr".   \
     ...
     C++ gives warning messages about overridings of the "oop_oop_iterate"     \
     defined above "hiding" this virtual function.  (DLD, 6/20/00)) */         \
  virtual int oop_oop_iterate##nv_suffix##_m(oop obj, OopClosureType *blk,     \
                                             MemRegion mr) {                   \
    return oop_oop_iterate_m(obj, blk, mr);                                    \
  }
