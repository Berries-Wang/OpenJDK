...
+ -XX:+UseParallelOldGC <sup>着重关注一下引用的 ‘ JDK8-ParallelOldGC作为Parallel默认Old-GC.pdf ’，这个是在2012年改了默认值 </sup>
  + 通过[Java-Parallel GC介绍-腾讯云开发者社区-腾讯云](./DOCS/Java-Parallel%20GC介绍-腾讯云开发者社区.pdf) 、 [JDK8-ParallelOldGC作为Parallel默认Old-GC.pdf](./DOCS/JDK8-ParallelOldGC作为Parallel默认Old-GC.pdf) <sup> (完整版原版截图: [JDK8-ParallelOldGC作为Parallel默认Old-GC.pdf](./DOCS/cloud.tencent.com_developer_article_1764615.png))</sup>可以发现，发现 ’Parallel Old‘ 出现前，Parallel Scavenge 搭配的Old GC是 'PSMarkSweep(Serial Old)' , 在JDK8中是 ’Parallel Old‘
