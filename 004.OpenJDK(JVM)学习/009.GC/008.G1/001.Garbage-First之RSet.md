...
2. Point In: 会在ObjB的RSet中记录ObjA的地址;(G1的选择)
   ...
   > Point In记录操作复杂，但是在标记扫描时可以直接找到有用和无用的对象，不需要进行额外的扫描，因为RSet中的对象可以看作为跟对象。
...
```
### Card Table · 卡表
&nbsp;&nbsp;以下会将到卡表，关于卡表，请参考:[004.OpenJDK(JVM)学习/009.GC/004.ParNew+CMS/012.CardTable.md](../../../004.OpenJDK(JVM)学习/009.GC/004.ParNew+CMS/012.CardTable.md)
