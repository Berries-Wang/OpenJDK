...
&nbsp;&nbsp; LT 与 ET 区别: 当一个新的时间到来时，ET模式下可以从epoll_wait调用中获取到这个事件，但是如果这次没有把这个事件对应的套接字缓冲区处理完，在这个套接字没有新的事件再次到来时，在ET模式下是无法再次从epoll_wait调用中获取到这个事件的。而LT模式则相反，只要有一个事件对应的套接字缓冲区还有数据，就总能从epoll_wait中获取到这个事件。
